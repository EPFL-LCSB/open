...
#from cobra.sampling import OptGPSampler, ACHRSampler, HRSampler, \
 #   shared_np_array
from open.sampling.achr_sampler import ACHRSampler
from open.sampling.optgp_sampler import OptGPSampler
from open.sampling.samplers import shared_np_array,HRSampler
...
class GeneralizedHRSampler(HRSampler):
    ...
    def __init__(self, model, thinning, nproj=None, seed=None):
        ...
        self.model = Model.clone(model)

        #newly added
        #self.model.solver = Model.clone(model)
    ...
    def generate_fva_warmup(self):
        ...
        self.model.objective = Objective(0, direction='max')
        ...
        for i in idx:
            ...
            slim_optimize(self.model)

            if not self.model.status == OPTIMAL:
